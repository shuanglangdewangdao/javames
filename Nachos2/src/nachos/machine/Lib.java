...
public final class Lib {
    ...
    private static Random random = null;   //Random生成随机数使用的类
     ...
     * 
     * 输入一个大于零的int数range，返回一个int类型在[0,range)的随机数
     * 
     */
    public static int random(int range) {
	assertTrue(range > 0);
	return random.nextInt(range);  //返回一个伪随机数，它是取自此随机数生成器序列的、在 0（包括）和range（不包括）之间均匀分布的 int 值。
    }
	...
	return random.nextDouble();  //返回下一个伪随机数，它是取自此随机数生成器序列的、在 0.0 和 1.0 之间均匀分布的 double 值。
    }
